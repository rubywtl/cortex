...
func (o *LogAlertLifeCycleObserver) AddedAggrGroup(alerts []*types.Alert, meta alertobserver.AlertEventMeta) {
	...
	groupId, ok := meta["groupId"]
	if !ok {
		return
	}
	for _, a := range alerts {
		o.logWithAlertWithLabels(a, "msg", "Added to aggregation group", "groupKey", groupKey, "groupId", groupId)
	}
}
...
func (o *LogAlertLifeCycleObserver) PipelineStart(alerts []*types.Alert, meta alertobserver.AlertEventMeta) {
	...
	groupId, ok := notify.GroupId(ctx.(context.Context))
	if !ok {
		return
	}
	if o.limiter.Level() < 5 {
		level.Info(o.logger).Log("msg", "Entered the pipeline", "groupId", groupId, "receiver", receiver, "alertsCount", len(alerts))
	} else {
		o.logWithAlertFingerprints(alerts, "msg", "Entered the pipeline", "groupId", groupId, "receiver", receiver)
	}
}
...
func (o *LogAlertLifeCycleObserver) PipelinePassStage(alerts []*types.Alert, meta alertobserver.AlertEventMeta) {
	...
	level.Info(o.logger).Log("msg", "Passed stage", "groupId", groupId, "receiver", receiver, "stage", stageName, "alertsCount", len(alerts))
}
...
func (o *LogAlertLifeCycleObserver) Sent(alerts []*types.Alert, meta alertobserver.AlertEventMeta) {
	...
	if o.limiter.Level() < 4 {
		level.Info(o.logger).Log("msg", "Sent", "groupId", groupId, "receiver", receiver, "integration", integration, "alertsCount", len(alerts))
	} else {
		o.logWithAlertFingerprints(alerts, "msg", "Sent", "groupId", groupId, "receiver", receiver, "integration", integration)
	}
}
...
func (o *LogAlertLifeCycleObserver) SendFailed(alerts []*types.Alert, meta alertobserver.AlertEventMeta) {
	...
	if o.limiter.Level() < 4 {
		level.Info(o.logger).Log("msg", "Send failed", "groupId", groupId, "receiver", receiver, "integration", integration, "alertsCount", len(alerts))
	} else {
		o.logWithAlertFingerprints(alerts, "msg", "Send failed", "groupId", groupId, "receiver", receiver, "integration", integration)
	}
}
...
func (o *LogAlertLifeCycleObserver) Muted(alerts []*types.Alert, meta alertobserver.AlertEventMeta) {
	...
	o.logWithAlertFingerprints(alerts, "msg", "Muted", "groupId", groupId)
}
