...
func TestLogAlertLifeCycleObserver(t *testing.T) {
	...
	logger := &FakeLogger{}
	alerts := []*types.Alert{
		{Alert: alert1, UpdatedAt: time.Now(), Timeout: false},
		{Alert: alert2, UpdatedAt: time.Now(), Timeout: false},
	}
	ctx := context.Background()
	ctx = notify.WithReceiverName(ctx, "rcv")
	ctx = notify.WithGroupKey(ctx, "key")
	ctx = notify.WithGroupId(ctx, "gid")
	...
	for _, tc := range []struct {
		...
		},
		{
			event:              alertobserver.EventAlertAddedToAggrGroup,
			alerts:             alerts,
			meta:               alertobserver.AlertEventMeta{"groupKey": "test", "groupId": "gid"},
			logLvl:             []int{1, 2, 3, 4, 5},
			expectedLogCount:   2,
		...
		},
		{
			...
			expectedLoggedKeys:  []string{"groupId", "receiver"},
		...
		},
		{
			...
			expectedLogCount:    1,
			expectedMsg:         "Entered the pipeline",
			expectedLoggedKeys:  []string{"groupId", "receiver", "fingerprints"},
		...
		},
		{
			...
			expectedLoggedKeys:  []string{"groupId", "receiver", "stage"},
		...
		},
		{
			...
			expectedLogCount:    1,
			expectedMsg:         "Muted",
			expectedLoggedKeys:  []string{"groupId", "fingerprints"},
			expectedMissingKeys: []string{"labels"},
			expectedKeyValues: []map[string]string{
				{
					"fingerprints": fmt.Sprintf("%v,%v", alert1.Fingerprint().String(), alert2.Fingerprint().String()),
