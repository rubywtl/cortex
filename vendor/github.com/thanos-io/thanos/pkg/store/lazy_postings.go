...
func fetchAndExpandPostingGroups(
	...
	if len(lazyMatchers) > 0 {
		logger := level.Info(r.logger)
		sb := strings.Builder{}
		for _, pg := range postingGroups {
			logger.Log("name", pg.name, "matchers", labelMatchersToString(&sb, pg.matchers), "cardinality", pg.cardinality, "add_keys", len(pg.addKeys), "remove_keys", len(pg.removeKeys), "existent_keys", pg.existentKeys, "lazy", pg.lazy)
		}
	}
...
func labelMatchersToString(sb *strings.Builder, matchers []*labels.Matcher) string {
	if sb == nil {
		sb = &strings.Builder{}
	} else {
		sb.Reset()
